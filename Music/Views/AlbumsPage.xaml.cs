...
public partial class AlbumsPage : ContentPage
{
    ...
    protected override async void OnAppearing()
    {
        ...
        if (ViewModel.Albums == null || ViewModel.Albums.Count == 0)
        {
    ...
    private async void OnEditClicked(object sender, EventArgs e)
    {
        ...
        page.AlbumSaved += async (s, album) =>
        {
            try
            {
                await ViewModel.UpdateAsync(album);
            }
            catch (Exception ex)
            {
                await DisplayAlert("Ошибка", $"Не удалось обновить альбом: {ex.Message}", "OK");
            }
        };
        await Navigation.PushAsync(page);
    }
    ...
    private async Task StopAndDisposeAsync(ImageButton btn)
    {
        try
        {
            _player?.Stop();
        ...
        {
            await DisplayAlert("Ошибка", $"Ошибка при остановке: {ex.Message}", "OK");
        }
    }
